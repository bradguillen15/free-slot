...
- Panel: an **in-flow, absolutely-positioned panel** anchored under the field (NOT a portaled Radix `Popover` and NOT a separate mobile modal). A portaled popover renders outside the dialog, where the dialog's `react-remove-scroll` lock can block wheel/touch scrolling and its focus-trap can block typing; and the dialog's `transform` can mis-position a `position: fixed` popper. The component therefore renders the panel within its own DOM subtree and handles outside-click + `Escape` (with `stopPropagation` so `Escape` doesn't also close the dialog). On mobile, the same panel expands to a partial-height sheet sized around 40-50% of the viewport, rather than fullscreen.
- The panel uses two infinite-feeling scroll-snap **wheels** for hours and minutes. The implementation repeats wheel values internally and recenters after scroll commit so users can continue past the first or last value without hitting a hard boundary. Clicking a row also selects it. Minutes use 5-minute steps (matches slot snapping granularity); an off-step current minute is included so existing values remain selectable and visibly selected.
