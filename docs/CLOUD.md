...
- **Email confirmation**: intentionally **off** (auto-confirm / instant signin) — signup logs the user straight in, keeping the guest→account transition fluid and avoiding any dependency on email deliverability. The signup-confirmation UI has been removed from `Auth.tsx`; the only auth email left is password recovery. Configure in Supabase dashboard → Authentication → Providers → Email → "Confirm email" toggle. Trade-off: a user can register an address they don't own (they still can't access it); acceptable while the app is a low-traffic personal project. Re-introducing confirmation (re-adding the inbox/resend flow + `signUpRedirectTo`) is the natural point to add MFA when scaling.
- **Redirect URLs (required for recovery links)**: the app passes `redirectTo` for password recovery built from `window.location.origin` (see `src/lib/authConfig.ts`). GoTrue only honors that target if it is in the **allowed Redirect URLs**; otherwise it silently falls back to the **Site URL**. So in Supabase dashboard → Authentication → URL Configuration, set **Site URL** to the production domain and add it (plus any preview domains) to **Redirect URLs**. A reset link that lands on `http://localhost…` in production means Site URL is still localhost and/or the prod domain is missing from the allowlist — it is a dashboard config issue, not a code bug.
