---
description: Backend standards for FreeSlot (plan-grow) — Lovable Cloud / Supabase, Postgres, RLS, Deno edge functions
...
FreeSlot's backend is **Lovable Cloud (Supabase)**: Postgres with RLS, Auth, and Deno edge functions. There is no separate Express/Node API server.
...
| AI | Lovable AI Gateway (no API keys in repo) |
| Migrations | `supabase/migrations/` (managed by Lovable Cloud) |
...
Regenerated by Lovable Cloud. Use migrations and types regeneration instead.
...
Auto-managed `.env` keys (do not commit):
