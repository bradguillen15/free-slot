...
describe("LabelsEditor — guest mode", () => {
  ...
  it("shows a productive default label inside the productive column", async () => {
    ...
    // The row lives inside the productive column (type is conveyed by the column,
    // not an inline select — type is changed by dragging between columns).
    const productiveColumn = screen.getByTestId("label-column-productive").parentElement!;
    expect(within(productiveColumn).getByTestId(`label-row-${deep.id}`)).toBeInTheDocument();
  });
